...
func (w *WalkedDepth5WS) writeLoop(ctx context.Context, conn *websocket.Conn) {
...
func (w *WalkedDepth5WS) readLoop(
	conn *websocket.Conn,
...
func (w *WalkedDepth5WS) mainLoop(
...
func (w *WalkedDepth5WS) heartbeatLoop(ctx context.Context, conn *websocket.Conn, symbols []string, pingInterval time.Duration) {
...
func (w *WalkedDepth5WS) dataHandleLoop(ctx context.Context, symbol string, impact float64, inputCh chan []byte, outputCh chan common.Ticker) {
	...
	for {
		...
		case msg := <-inputCh:
			...
			if err != nil {
				logger.Debugf("common.WalkDepth error %v %s", err, msg)
				continue
			}
			select {
			...
			default:
				if time.Now().Sub(logSilentTime) > 0 {
					logger.Debugf("outputCh <- walkedDepth5 failed, ch len %d", len(outputCh))
					logSilentTime = time.Now().Add(time.Minute)
				}
			}
		}
