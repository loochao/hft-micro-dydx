...
func (w *RawTradeWS) readLoop(
	...
	for {
		...
		if msgLen > 128 {
			if msg[2] == 't' {
				if msg[62] == ',' {
					symbol = common.UnsafeBytesToString(msg[53:61])
				} else if msg[63] == ',' {
					symbol = common.UnsafeBytesToString(msg[53:62])
				} else if msg[64] == ',' {
					symbol = common.UnsafeBytesToString(msg[53:63])
				} else if msg[61] == ',' {
					symbol = common.UnsafeBytesToString(msg[53:60])
				} else if msg[65] == ',' {
					symbol = common.UnsafeBytesToString(msg[53:64])
				} else {
...
func (w *RawTradeWS) heartbeatLoop(ctx context.Context, conn *websocket.Conn, symbols []string, pingInterval time.Duration) {
			...
			for symbol, updateTime := range symbolUpdatedTimes {
				if time.Now().Sub(updateTime) > symbolTimeout {
					...
					case <-ctx.Done():
						return
					case <-time.After(time.Millisecond):
						logger.Debugf("send msg to writeCh timeout in 1m, %s", fmt.Sprintf("/contractMarket/execution:%s", symbol))
