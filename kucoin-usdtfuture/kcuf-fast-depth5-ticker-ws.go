...
const (
	...
	depth5TickerReadPoolBufferSize = 1024
)

type FastDepth5TickerWS struct {
	...
	stopped       int32
	readPool      [depth5TickerReadPoolSize][]byte
	readPoolIndex int
}

func (w *FastDepth5TickerWS) writeLoop(ctx context.Context, conn *websocket.Conn) {
...
func (w *FastDepth5TickerWS) readLoop(
	conn *websocket.Conn,
...
func (w *FastDepth5TickerWS) readAll(r io.Reader) ([]byte, error) {
	w.readPoolIndex++
	if w.readPoolIndex == depth5TickerReadPoolSize {
		w.readPoolIndex = 0
	}
	b := w.readPool[w.readPoolIndex]
	n, err := r.Read(b)
	if err == nil {
		if n < depth5TickerReadPoolBufferSize {
			return b[:n], err
		}else{
			for {
				if len(b) == cap(b) {
				...
				if err != nil {
					if err == io.EOF {
						err = nil
						logger.Debugf("can't read %d int %d, msg: %s", len(b), depth5TickerReadPoolBufferSize, b)
					}
					return b, err
				}
			}
...
func (w *FastDepth5TickerWS) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *FastDepth5TickerWS) mainLoop(
...
func (w *FastDepth5TickerWS) heartbeatLoop(ctx context.Context, conn *websocket.Conn, symbols []string, pingInterval time.Duration) {
...
func (w *FastDepth5TickerWS) Stop() {
...
func (w *FastDepth5TickerWS) restart() {
	select {
	case <-w.done:
	case <-time.After(time.Millisecond):
		w.Stop()
...
func (w *FastDepth5TickerWS) dataHandleLoop(ctx context.Context, symbol string, inputCh chan []byte, outputCh chan common.Ticker) {
...
func (w *FastDepth5TickerWS) Done() chan interface{} {
	return w.done
}

func NewFastDepth5TickerWS(
	ctx context.Context,
	api *API,
	proxy string,
	channels map[string]chan common.Ticker,
) *FastDepth5TickerWS {
	ws := FastDepth5TickerWS{
	...
	for i := range ws.readPool {
		ws.readPool[i] = make([]byte, depth5TickerReadPoolBufferSize, depth5TickerReadPoolBufferSize)
	}
