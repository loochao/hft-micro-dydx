	...
	for offset < bytesLen-2 {
		...
		case common.JsonKeyPrice:
			if bytes[offset] == '"' {
				...
				if err != nil {
					return nil, fmt.Errorf("JsonKeyPrice error %v mainLoop %d end %d %s", err, collectStart, offset, bytes[collectStart:offset])
				}
				currentKey = common.JsonKeyUnknown
				offset += 3
				continue
		...
		case common.JsonKeyQuantity:
			if bytes[offset] == '"' {
				...
				if err != nil {
					return nil, fmt.Errorf("JsonKeyQuantity error %v mainLoop %d end %d %s", err, collectStart, offset, bytes[collectStart:offset])
				}
				currentKey = common.JsonKeyUnknown
				offset = bytesLen
				continue
	...
	for offset < bytesLen-2 {
		...
		case common.JsonKeyMarkPrice:
			if bytes[offset] == '"' {
				...
				if err != nil {
					return nil, fmt.Errorf("JsonKeyMarkPrice error %v mainLoop %d end %d %s", err, collectStart, offset, bytes[collectStart:offset])
				}
				currentKey = common.JsonKeyUnknown
				offset += 3
				continue
		...
		case common.JsonKeyIndexPrice:
			if bytes[offset] == '"' {
				...
				if err != nil {
					return nil, fmt.Errorf("JsonKeyIndexPrice error %v mainLoop %d end %d %s", err, collectStart, offset, bytes[collectStart:offset])
				}
				currentKey = common.JsonKeyUnknown
				offset += 3
				continue
		...
		case common.JsonKeyEstimatedSettlePrice:
			if bytes[offset] == '"' {
				...
				if err != nil {
					return nil, fmt.Errorf("JsonKeyEstimatedSettlePrice error %v mainLoop %d end %d %s", err, collectStart, offset, bytes[collectStart:offset])
				}
				currentKey = common.JsonKeyUnknown
				offset += 3
				continue
		...
		case common.JsonKeyFundingRate:
			if bytes[offset] == '"' {
				...
				if err != nil {
					return nil, fmt.Errorf("JsonKeyFundingRate error %v mainLoop %d end %d %s", err, collectStart, offset, bytes[collectStart:offset])
				}
				currentKey = common.JsonKeyUnknown
				offset += 3
				continue
		...
		case common.JsonKeyUnknown:
			if bytes[offset] == 'E' && bytes[offset-1] == '"' && bytes[offset+1] == '"' && offset+16 < bytesLen {
				...
				if err != nil {
					return nil, fmt.Errorf("NextFundingTime error %v mainLoop %d end %d %s", err, collectStart, offset, bytes[collectStart:offset])
				}
