...
func (strat *XYStrategy) updateSpread() {
	...
	strat.shortLastEnter = (strat.yTicker.GetBidPrice()*strat.yExchange.GetPriceFactor() - strat.xTicker.GetAskPrice()*strat.xExchange.GetPriceFactor()) / (strat.xTicker.GetAskPrice()*strat.xExchange.GetPriceFactor())
	strat.longLastEnter = (strat.yTicker.GetAskPrice()*strat.yExchange.GetPriceFactor() - strat.xTicker.GetBidPrice()*strat.xExchange.GetPriceFactor()) / (strat.xTicker.GetBidPrice()*strat.xExchange.GetPriceFactor())
	...
	if strat.spreadTime.Sub(strat.quantileLastSampleTime) > strat.config.QuantileSampleInterval {
		...
		*strat.quantileMiddle = strat.timedTDigest.Quantile(0.5)
		strat.quantileEnterBot = strat.timedTDigest.Quantile(strat.config.SpreadLongEnterQuantileBot)
		strat.quantileEnterTop = strat.timedTDigest.Quantile(strat.config.SpreadShortEnterQuantileTop)
		strat.quantileExitBot = strat.timedTDigest.Quantile(strat.config.SpreadShortExitQuantileBot)
		strat.quantileExitTop = strat.timedTDigest.Quantile(strat.config.SpreadLongExitQuantileTop)
