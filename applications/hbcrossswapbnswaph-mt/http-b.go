...
func handleBnswapHttpPositions(positions []bnswap.Position) {
	for _, nextPos := range positions {
		if _, ok := bhSymbolsMap[nextPos.Symbol]; !ok {
			return
		}
		if nextPos.PositionSide != "BOTH" {
			return
		}
		if nextPos.UpdateTime.Sub(bLastOrderTimes[nextPos.Symbol]) < *hbConfig.PullInterval {
			return
		}
		var lastPosition *bnswap.Position
		if p, ok := bPositions[nextPos.Symbol]; ok {
			p := p
			lastPosition = &p
		}
