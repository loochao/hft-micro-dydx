...
func handleWSAccountEvent(data *bnswap.WSAccountEvent) {
	for _, pos := range data.Account.Positions {
		if _, ok := boSymbolsMap[pos.Symbol]; !ok {
			continue
		}
		...
		bnswapPositionsUpdated[pos.Symbol] = true
		...
		if lastPosition == nil ||
			...
			//bnswapOrderSilentTimes[pos.Market] = time.Now()
