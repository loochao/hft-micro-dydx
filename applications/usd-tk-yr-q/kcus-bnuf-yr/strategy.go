...
func strategyA(
	...
	for _, spread := range data {
		...
		if ignore {
			//数据有错，直接平仓
			if yPosition.Size > 0 {
				tradeVolume += math.Abs(yPosition.Size * *lastYBidPrice)
				currentYValue += yPosition.Size * *lastYBidPrice * params.TradeCost
				currentYValue += yPosition.Add(-yPosition.Size, *lastYBidPrice)
			}else if yPosition.Size < 0 {
				tradeVolume += math.Abs(yPosition.Size * *lastYAskPrice)
				currentYValue += -yPosition.Size * *lastYAskPrice * params.TradeCost
				currentYValue += yPosition.Add(-yPosition.Size, *lastYAskPrice)
			}
			continue
		}
		*lastXAskPrice = spread.XAskPrice
