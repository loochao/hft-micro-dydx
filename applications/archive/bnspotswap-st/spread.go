...
func watchMakerTakerSpread(
	...
	for {
		...
		case <-walkSpreadTimer.C:
			...
			matchCount++
			shortLastEnter = (takerWalkedDepth.TakerBid - makerWalkedDepth.TakerAsk) / makerWalkedDepth.TakerAsk
			longLastEnter = (takerWalkedDepth.TakerAsk - makerWalkedDepth.TakerBid) / makerWalkedDepth.TakerBid
			...
			case outputCh <- &common.ShortSpread{
				...
				LastLeave:   longLastEnter,
				MedianEnter: shortEnterSortedSlice.Median(),
				MedianLeave: longEnterSortedSlice.Median(),
		...
		case <-makerParseTimer.C:
			...
			if err != nil {
				if time.Now().Sub(logSilentTime) > 0 {
					logger.Debugf("bnspot.ParseDepth20 error %v %s %s", err, makerSymbol, makerRawDepth.Depth)
					logSilentTime = time.Now().Add(time.Minute)
				}
