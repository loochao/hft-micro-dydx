...
func (strat *XYStrategy) walkXDepth() {
	strat.error = common.WalkDepthBMA(strat.xDepth, strat.params.xMultiplier, strat.params.depthTakerImpact, &strat.xWalkedDepth)
	if strat.error != nil {
...
func (strat *XYStrategy) walkYDepth() {
	strat.error = common.WalkDepthBMA(strat.yDepth, strat.params.yMultiplier, strat.params.depthTakerImpact, &strat.yWalkedDepth)
	if strat.error != nil {
		if time.Now().Sub(strat.logSilentTime) > 0 {
