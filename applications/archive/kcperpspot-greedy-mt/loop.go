...
func updatePerpPositions() {
	...
	for _, spotSymbol := range kcspotSymbols {
		...
		unHedgedValue += math.Abs(perpSize * spread.TakerDepth.MidPrice)
		...
		price := math.Round(perpOrderBook.MidPrice*(1.0+*kcConfig.EnterSlippage)/perpTickSize) * perpTickSize
...
func updateSpotNewOrders() {
	...
	for rank := len(kcperpSymbols) - 1; rank >= 0; rank-- {
		...
		if spread.LastLeave < quantile.Bot &&
			spread.MedianLeave < quantile.Bot &&
		...
		} else if spread.LastEnter > quantile.Top &&
