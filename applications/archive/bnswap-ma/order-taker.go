...
func isTakerOrderOk(order bnswap.NewOrderParams) bool {
	takerDepth, ok1 := mtDepths[order.Symbol]
	takerSignal, ok3 := mtSignals[order.Symbol]
	takerPosition, ok2 := tPositions[order.Symbol]
	if !ok1 || !ok2 || !ok3 || time.Now().Sub(takerDepth.Time) > *mtConfig.SpreadTimeToLive {
		if !ok1 || !ok2 {
	...
	if (takerSignal.Direction > 0 && takerPosition.PositionAmt <= 0) ||
		(takerSignal.Direction < 0 && takerPosition.PositionAmt >= 0) {
		//检查价格有没有挂太远，太远撤掉
		if order.Side == common.OrderSideBuy &&
			order.Price < takerDepth.BestBidPrice {
			//logger.Debugf("TAKER BUY %s %f < BEST BID %f",
			//	order.Market,
			//	order.Price,
			//	takerDepth.TakerDepth.BestBidPrice,
			//)
			return false
			...
			//	order.Market,
			//	order.Price,
			//	takerDepth.TakerDepth.TakerFarBid,
			//)
			return false
		}
	} else {
