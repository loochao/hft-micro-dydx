...
func main() {
	...
	mtInternalInfluxWriter, err = common.NewInfluxWriter(
		mtGlobalCtx,
	...
	defer mtInternalInfluxWriter.Stop()
	...
	for {
		...
		case msg := <-tUserWebsocket.BalanceAndPositionUpdateEventCh:
			...
			break
		case msg := <-mUserWebsocket.BalancesCh:
