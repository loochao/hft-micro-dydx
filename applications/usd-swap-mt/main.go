...
func main() {
	...
	xOrderOffsets := make(map[string]Offset)
	yOrderOffsets := make(map[string]Offset)
	for xSymbol, ySymbol := range xyConfig.XYPairs {
		...
		xOrderOffsets[xSymbol], err = NewOffset(xyConfig.XOrderOffsets[xSymbol])
		if err != nil {
			logger.Debugf("NewXOffset error %s %v", xSymbol, err)
			return
		}
		yOrderOffsets[ySymbol], err = NewOffset(xyConfig.YOrderOffsets[ySymbol])
		if err != nil {
			logger.Debugf("NewYOffset error %s %v", ySymbol, err)
			return
		}
	}
	...
	for xSymbol, ySymbol := range xyConfig.XYPairs {
		err = startXYStrategy(
			...
			xSymbol, ySymbol,
			*xyConfig,
			xOrderOffsets[xSymbol],
			yOrderOffsets[ySymbol],
			xExchange,
			yExchange,
