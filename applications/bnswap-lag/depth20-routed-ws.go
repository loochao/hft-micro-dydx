...
func (w *Depth20RoutedWebsocket) startRead(conn *websocket.Conn, decay, bias float64, symbols []string, bidPriceCh chan BidPrice, timeDeltaCh chan float64) {
	...
	var ok bool
	var t int64
	var symbolsMap = make(map[string]*BidPrice)
	for _, symbol = range symbols {
		symbolsMap[symbol] = &BidPrice{Symbol: symbol}
	}
	start := 0
	collectStart := 0
	for {
		...
		if _, ok = symbolsMap[symbol]; ok {
			collectStart = 0
			for start = 113; start < len(msg)-320; start++ {
				if msg[start] == '"' && msg[start-1] == 'b' && msg[start-2] == '"' {
					start += 5
					collectStart = start
				} else if collectStart != 0 && msg[start] == '"' {
					symbolsMap[symbol].Price, err = common.ParseFloat(msg[collectStart:start])
					if err != nil {
						if time.Now().Sub(logSilentTime) > 0 {
							logger.Debugf("common.ParseFloat bid price error %v msg %s", err, msg[collectStart:start])
							logSilentTime = time.Now().Add(time.Minute)
						}
					} else {
						select {
						case bidPriceCh <- *symbolsMap[symbol]:
						default:
						}
					}
					break
