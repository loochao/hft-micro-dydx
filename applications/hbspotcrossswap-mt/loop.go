...
func updatePerpPositions() {
	...
	for _, spotSymbol := range hbspotSymbols {
		swapSymbol := hbSpotSwapSymbolsMap[spotSymbol]
		...
		unHedgedValue += math.Abs(swapSize * spread.TakerDepth.TakerAsk)
...
func updateSpotNewOrders() {
	...
	for rank := len(hbcrossswapSymbols) - 1; rank >= 0; rank-- {
		...
		if time.Now().Sub(spread.Time) > *hbConfig.SpreadTimeToLive {
			continue
		}
		...
		mergedStepSize := hbMergedStepSizes[spotSymbol]
		...
		if spread.ShortLastLeave < quantile.Bot &&
			...
			if spotBalance.Available*price > spotMinNotional {
				...
				if amount > 0 && amount*price > spotMinNotional{
					...
					order := hbspot.NewOrderParam{
						...
						OriginPrice:   price,
						OriginAmount:  amount,
						Type:          hbspot.OrderTypeSellLimitMaker,
					}
					hbspotOrderSilentTimes[spotSymbol] = time.Now().Add(*hbConfig.OrderSilent)
			...
			if entryValue < entryStep*0.8 {
				if time.Now().Sub(hbLogSilentTimes[spotSymbol]) > 0 {
					...
					hbLogSilentTimes[spotSymbol] = time.Now().Add(time.Minute * 5)
				}
				continue
			}
			...
			order := hbspot.NewOrderParam{
				...
				OriginPrice:   price,
				OriginAmount:  amount,
				Type:          hbspot.OrderTypeBuyLimitMaker,
			}
