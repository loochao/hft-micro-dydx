...
func main() {
	...
	klinesChs := make(map[string]chan common.KLine)
	for start := 0; start < len(tSymbols); start += *mtConfig.OrderBookBatchSize {
		end := start + *mtConfig.OrderBookBatchSize
		if end > len(tSymbols) {
			end = len(tSymbols)
		}
		subMakerRowDepthChs := make(map[string]chan common.KLine)
		for _, symbol := range tSymbols[start:end] {
			klinesChs[symbol] = make(chan common.KLine, 100)
			subMakerRowDepthChs[symbol] = klinesChs[symbol]
		}
		go takerRoutedKlineLoop(
			mtGlobalCtx,
			mtGlobalCancel,
			*mtConfig.ProxyAddress,
			subMakerRowDepthChs,
	...
	depthCh := make(chan *common.WalkedTakerDepth, len(tSymbols)*100)
	...
	for {
		...
		case takerOrderEvent := <-tUserWebsocket.OrderUpdateEventCh:
			...
			break
		case depth := <-depthCh:
			mtDepths[depth.Symbol] = depth
			break
		case ss := <-mtSignalCh:
			mtSignals[ss.Symbol] = ss
			logger.Debugf("%v", ss)
