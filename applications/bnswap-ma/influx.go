...
func handleSave() {
	...
	for _, takerSymbol := range tSymbols {
		...
		if takerPosition, ok := tPositions[takerSymbol]; ok {
			...
			if spread, ok := mtDepths[takerSymbol]; ok {
				fields["takerValue"] = spread.TakerBid * takerPosition.PositionAmt
			}
		}
		if spread, ok := mtDepths[takerSymbol]; ok {

			fields["takerTakerBid"] = spread.TakerBid
			fields["takerTakerAsk"] = spread.TakerAsk
		}
		...
		if signal, ok := mtSignals[takerSymbol]; ok {
			fields["direction"] = signal.Direction
			fields["fast"] = signal.Fast
			fields["slow"] = signal.Slow
			fields["close"] = signal.Close
		}
		pt, err := client.NewPoint(
			*mtConfig.InternalInflux.Measurement,
			map[string]string{
