...
func main() {
	...
	//symbols = symbols[1:2]
	symbols = []string{"AXS-USDT"}
	startTime, err := time.Parse("20060102", "20210725")
	if err != nil {
		logger.Fatal(err)
	}
	endTime, err := time.Parse("20060102", "20210730")
	if err != nil {
		logger.Fatal(err)
	}
	dateStrs := ""
	...
	quantilePath := "/Users/chenjilin/Projects/hft-micro/applications/usd-tk-tt-q/configs/kcus-bnuf-quantiles"
	dataPath := "/Volumes/MarketData/MarketData/kcus-bnuf-depth5-and-ticker"
	...
	fmt.Printf("\n\nxyPairs:\n")
	for _, xSymbol := range symbols {
		fmt.Printf("  %s: %s\n", xSymbol, strings.Replace(xSymbol, "-USDT", "USDT", -1))
	}
	qSum := 0.0
	fmt.Printf("\n\nmaxOrderValues:\n")
	for _, xSymbol := range symbols {
