...
func main() {
	...
	for symbol := range kucoin_usdtfuture.TickSizes {
		if _, ok := binance_busdspot.TickSizes[strings.Replace(symbol, "USDTM", "BUSD", -1)]; ok {
			symbols = append(symbols, symbol)
			symbolsMap[symbol] = strings.Replace(symbol, "USDTM", "BUSD", -1)
		}
	}
	...
	return
	startTime, err := time.Parse("20060102", "20210822")
	if err != nil {
		logger.Fatal(err)
	}
	endTime, err := time.Parse("20060102", "20210824")
	if err != nil {
		logger.Fatal(err)
	}
	dateStrs := ""
	...
	quantilePath := "/Users/chenjilin/Projects/hft-micro/applications/usd-tk-tt-q/configs/kcuf-bnbs-quantiles"
	...
	for _, xSymbol := range symbols {
		...
		for _, dateStr := range strings.Split(dateStrs, ",") {
			logger.Debugf("%s %s %s", xSymbol, dateStr, fmt.Sprintf("/Users/chenjilin/MarketData/kcuf-bnbs-depth5-and-ticker/%s/%s-%s,%s.jl.gz", dateStr, dateStr, xSymbol, ySymbol))
			file, err := os.Open(
				fmt.Sprintf("/Users/chenjilin/MarketData/kcuf-bnbs-depth5-and-ticker/%s/%s-%s,%s.jl.gz", dateStr, dateStr, xSymbol, ySymbol),
			)
			if err != nil {
			...
			for scanner.Scan() {
				...
				if msg[0] == 'X' && msg[1] == 'T' {
					...
					err = binance_busdspot.ParseBookTicker(msg[21:], yTicker)
					if err != nil {
						logger.Debugf("%v", err)
						continue
					}
