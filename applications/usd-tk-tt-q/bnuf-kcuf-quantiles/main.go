...
func main() {
	...
	//symbols = symbols[:1]
	startTime, err := time.Parse("20060102", "20210818")
	if err != nil {
		logger.Fatal(err)
	}
	endTime, err := time.Parse("20060102", "20210826")
	if err != nil {
		logger.Fatal(err)
	}
	dateStrs := ""
	...
	quantilePath := "/Users/chenjilin/Projects/hft-micro/applications/usd-tk-tt-q/configs/bnuf-kcuf-quantiles"
	...
	for _, xSymbol := range symbols {
		...
		xTicker := &binance_usdtfuture.BookTicker{}
		xDepth := &binance_usdtfuture.Depth5{}
		...
		for _, dateStr := range strings.Split(dateStrs, ",") {
			logger.Debugf("%s %s %s", xSymbol, dateStr, fmt.Sprintf("/Users/chenjilin/MarketData/bnuf-kcuf-depth5-and-ticker/%s/%s-%s,%s.jl.gz", dateStr, dateStr, ySymbol, xSymbol))
			file, err := os.Open(
				fmt.Sprintf("/Users/chenjilin/MarketData/kcuf-bnuf-depth5-and-ticker/%s/%s-%s,%s.jl.gz", dateStr, dateStr, ySymbol, xSymbol),
			)
			if err != nil {
			...
			for scanner.Scan() {
				...
				msg = scanner.Bytes()
				if msg[0] == 'B' && msg[1] == 'T' {
					err = binance_usdtfuture.ParseBookTicker(msg[21:], xTicker)
					if err != nil {
						logger.Debugf("%v", err)
						continue
					}
					...
					err = binance_usdtfuture.ParseDepth5(msg[21:], xDepth)
					if err != nil {
						logger.Debugf("%v", err)
						continue
					}
				...
				if yTD != nil && xTD != nil {
					...
					_ = sizeTD.Add(math.Min(xTD.GetBidSize()*xTD.GetBidPrice(), yTD.GetAskSize()*yTD.GetAskPrice()*kucoin_usdtfuture.Multipliers[ySymbol]))
