...
func main() {
	...
	for symbol := range kucoin_usdtfuture.TickSizes {
		if _, ok := binance_usdtspot.TickSizes[strings.Replace(symbol, "USDTM", "USDT", -1)]; ok {
	...
	//symbols = symbols[:1]
	startTime, err := time.Parse("20060102", "20210820")
	if err != nil {
		logger.Fatal(err)
	}
	endTime, err := time.Parse("20060102", "20210827")
	if err != nil {
		logger.Fatal(err)
	}
	dateStrs := ""
	...
	quantilePath := "/Users/chenjilin/Projects/hft-micro/applications/usd-tk-tt-q/configs/kcuf-bnus-quantiles"
	...
	for _, xSymbol := range symbols {
		...
		yDepth := &binance_usdtspot.Depth5{}
		yTicker := &binance_usdtspot.BookTicker{}
		...
		for _, dateStr := range strings.Split(dateStrs, ",") {
			...
			for scanner.Scan() {
				...
				if msg[0] == 'X' && msg[1] == 'T' {
					...
					err = binance_usdtspot.ParseDepth5(msg[21:], yDepth)
					if err != nil {
						logger.Debugf("%v", err)
						continue
					}
					...
					err = binance_usdtspot.ParseTicker(msg[21:], yTicker)
					if err != nil {
						logger.Debugf("%v", err)
						continue
					}
