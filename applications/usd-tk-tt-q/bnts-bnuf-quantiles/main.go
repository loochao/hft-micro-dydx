...
func main() {
	...
	startTime, err := time.Parse("20060102", "20210801")
	if err != nil {
		logger.Fatal(err)
	}
	endTime, err := time.Parse("20060102", "20210804")
	if err != nil {
		logger.Fatal(err)
	}
	dateStrs := ""
	...
	quantilePath := "/Users/chenjilin/Projects/hft-micro/applications/usd-tk-tt-q/bnts-bnuf-quantiles/outputs"
	dataPath := "/Volumes/MarketData/MarketData/bnts-bnuf-depth-and-ticker"
	...
	for _, xSymbol := range symbols {
		...
		xDepth := &binance_tusdspot.Depth5{}
		xTicker := &binance_tusdspot.BookTicker{}
		...
		for _, dateStr := range strings.Split(dateStrs, ",") {
			...
			for scanner.Scan() {
				...
				msg = scanner.Bytes()
				if msg[0] == 'X' && msg[1] == 'D' {
					err = binance_tusdspot.ParseDepth5(msg[21:], xDepth)
					if err != nil {
						logger.Debugf("%v", err)
						continue
					}
					...
					err = binance_tusdspot.ParseBookTicker(msg[21:], xTicker)
					if err != nil {
						logger.Debugf("%v", err)
						continue
					}
