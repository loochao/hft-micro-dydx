...
func main() {
	...
	symbols := []string{"BTCBUSD"}
	for symbol := range binance_busdspot.TickSizes {
		if _, ok := kucoin_usdtfuture.TickSizes[strings.Replace(symbol, "BUSD", "USDTM", -1)]; ok {
			symbols = append(symbols, symbol)
			symbolsMap[symbol] = strings.Replace(symbol, "BUSD", "USDTM", -1)
		}
	}
	...
	quantilePath := "/Users/chenjilin/Projects/hft-micro/applications/usd-tk-tt-q/configs/bnbs-kcuf-quantiles"
	...
	for _, xSymbol := range symbols {
		...
		for _, dateStr := range strings.Split(dateStrs, ",") {
			logger.Debugf("%s %s %s", xSymbol, dateStr, fmt.Sprintf("/Users/chenjilin/MarketData/kcuf-bnbs-depth5-and-ticker/%s/%s-%s,%s.jl.gz", dateStr, dateStr, ySymbol, xSymbol))
			file, err := os.Open(
				fmt.Sprintf("/Users/chenjilin/MarketData/kcuf-bnbs-depth5-and-ticker/%s/%s-%s,%s.jl.gz", dateStr, dateStr, ySymbol, xSymbol),
			)
			if err != nil {
			...
			for scanner.Scan() {
				...
				if yTD != nil && xTD != nil {
					...
					if tDiff < maxTimeDiff &&
						...
						_ = sizeTD.Add(
							math.Min(
								8*math.Min(xTD.GetBidSize()*xTD.GetBidPrice(), 8*xTD.GetAskSize()*xTD.GetAskPrice()),
