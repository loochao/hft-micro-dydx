...
func (strat *XYStrategy) updateXOrder() {
	...
	if strat.spread.ShortLastLeave < strat.shortBot &&
		...
		*strat.xyFundingRate < strat.config.MinimalKeepFundingRate &&
		...
		strat.enterValue = math.Min(math.Min(4*strat.enterStep, strat.xAbsValue), math.Min(strat.xAbsValue, strat.yAbsValue))
		if *strat.xyFundingRate > strat.config.MinimalKeepFundingRate*0.5 {
			strat.enterValue = math.Min(math.Min(2*strat.enterStep, strat.xAbsValue*0.5), math.Min(strat.xAbsValue, strat.yAbsValue))
		}
		...
		if strat.size > 0 && (!strat.isXSpot || strat.enterValue > 1.2*strat.xMinNotional) {
			...
			if strat.price < strat.xWalkedDepth.BestBidPrice {
				strat.price = strat.xWalkedDepth.BestBidPrice
			}
		...
		*strat.xyFundingRate > -strat.config.MinimalKeepFundingRate &&
		...
		strat.enterValue = math.Min(math.Min(4*strat.enterStep, strat.xAbsValue), math.Min(strat.xAbsValue, strat.yAbsValue))
		if *strat.xyFundingRate > strat.config.MinimalKeepFundingRate*0.5 {
			strat.enterValue = math.Min(math.Min(1*strat.enterStep, strat.xAbsValue*0.5), math.Min(strat.xAbsValue, strat.yAbsValue))
		}
		...
		if strat.size > 0 && (!strat.isXSpot || strat.enterValue > 1.2*strat.xMinNotional) {
			...
			if strat.price > strat.xWalkedDepth.BestAskPrice {
				strat.price = strat.xWalkedDepth.BestAskPrice
			}
			strat.xNewOrderParam = common.NewOrderParam{
				Symbol:      strat.xSymbol,
...
func (strat *XYStrategy) isXOpenOrderOk() bool {
	...
	if strat.xOpenOrder.Side == common.OrderSideBuy &&
		...
		logger.Debugf("%s CANCEL, BUY PRICE %f < BEST BID %f",
			strat.xSymbol,
		...
		logger.Debugf("%s CANCEL, SELL PRICE %f > BEST ASK %f",
			strat.xSymbol,
			strat.xOpenOrder.Price,
