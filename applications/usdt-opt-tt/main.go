...
func main() {
	...
	for _, xSymbol := range xSymbols {
		...
		xDepthChMap[xSymbol] = make(chan common.Depth)
		xOrderRequestChMap[xSymbol] = make(chan common.OrderRequest)
	...
	for _, ySymbol := range ySymbols {
		...
		yDepthChMap[ySymbol] = make(chan common.Depth)
		yOrderRequestChMap[ySymbol] = make(chan common.OrderRequest)
