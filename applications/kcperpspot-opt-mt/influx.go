		...
		if spotBalance, ok := kcspotBalances[spotSymbol]; ok {
			...
			if spread, ok := kcSpreads[spotSymbol]; ok {
				...
				if entryTarget != 0.0 {
					...
					//offset := kcspotOffsets[spotSymbol]
					fields["enterDelta"] = *kcConfig.EnterDelta + *kcConfig.OffsetDelta*(currentSpotValue/entryTarget)
					fields["exitDelta"] = *kcConfig.ExitDelta + *kcConfig.OffsetDelta*(currentSpotValue/entryTarget)
				}
			}
		}
