	...
	if strat.xLastFilledBuyPrice != nil && strat.yLastFilledSellPrice != nil {
		...
		*strat.realisedSpread = (*strat.yLastFilledSellPrice - *strat.xLastFilledBuyPrice) / *strat.yLastFilledSellPrice
		...
		if strat.tdBidSpreadMiddle != 0 {
			...
			*strat.adjustedRealisedSpread = *strat.realisedSpread - strat.tdBidSpreadMiddle
		}
		...
		if strat.tdBidSpreadMiddle != 0 &&
			...
			strat.xFundingRateFactor != nil {
			logger.Debugf("%10s - %10s realised short abs spread %f slippage x %f y %f quantile middle %f funding rate offset %f adjusted spread %f", strat.ySymbol, strat.xSymbol, *strat.realisedSpread, strat.xSlippage, strat.ySlippage, strat.tdBidSpreadMiddle, *strat.xyFundingRate, *strat.realisedSpread-strat.tdBidSpreadMiddle+*strat.xyFundingRate)
		} else {
