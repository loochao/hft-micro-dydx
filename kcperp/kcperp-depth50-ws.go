...
func (w *Depth50Websocket) startWrite(ctx context.Context, conn *websocket.Conn) {
	for {
		select {
		case <-ctx.Done():
			return
...
func (w *Depth50Websocket) startRead(conn *websocket.Conn) {
	totalCount := 0
	totalLen := 0
	for {
...
func (w *Depth50Websocket) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 1024)
	for {
		if len(b) == cap(b) {
...
func (w *Depth50Websocket) startDataHandler(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
		...
		case msg := <-w.messageCh:
			if msg[2] == 'd' && msg[5] == 'a' {
				depth50, err := ParseDepth50(msg)
				if err != nil {
					logger.Debugf("ParseDepth50 error %v %s", err, msg)
					continue
				}
				select {
				case <-ctx.Done():
				...
				case <-w.done:
					return
				case <-time.After(time.Millisecond):
					logger.Warn("KCPERP DEPTH20 TO OUTPUT CH TIME OUT IN 1MS")
...
func (w *Depth50Websocket) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *Depth50Websocket) start(ctx context.Context, api *API, symbols []string, proxy string) {
...
func (w *Depth50Websocket) maintainHeartbeat(ctx context.Context, conn *websocket.Conn, symbols []string, pingInterval time.Duration) {
		...
		case <-symbolCheckTimer.C:
			for symbol, updateTime := range symbolUpdatedTimes {
				if time.Now().Sub(updateTime) > symbolTimeout {
					logger.Debugf("KCPERP SUBSCRIBE %s", fmt.Sprintf("/contractMarket/level2Depth50:%s", symbol))
					select {
					case <-ctx.Done():
						return
					case <-time.After(time.Millisecond):
						logger.Debugf("SEND SUBSCRIBE %s TO WRITE TIMEOUT IN 1MS", fmt.Sprintf("/contractMarket/level2Depth50:%s", symbol))
						break
					case w.writeCh <- SubscribeMsg{
						ID:             fmt.Sprintf("/contractMarket/level2Depth50:%s", symbol),
						Type:           "subscribe",
...
func (w *Depth50Websocket) restart() {
...
func (w *Depth50Websocket) Done() chan interface{} {
	return w.done
}

func NewDepth50Websocket(
	ctx context.Context,
	api *API,
	symbols []string,
	proxy string,
