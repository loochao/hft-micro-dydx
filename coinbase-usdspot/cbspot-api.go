...
func (api *API) SendHTTPRequest(ctx context.Context, method, path string, params common.Params, result interface{}) error {
	path = "https://api.pro.coinbase.com" + path
	values := url.Values{}
	var err error
	if params != nil {
...
func (api *API) GetProducts(ctx context.Context) ([]Product, error) {
	products := make([]Product, 0)
	return products, api.SendHTTPRequest(ctx, http.MethodGet, "/products", nil, &products)
}
