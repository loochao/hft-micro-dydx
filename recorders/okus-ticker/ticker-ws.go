...
func (w *TickerWS) writeLoop(ctx context.Context, conn *websocket.Conn) {
	...
	for {
		...
		case msg := <-w.writeCh:
			...
			err = conn.WriteMessage(websocket.TextMessage, _msg)
			if err != nil {
...
func (w *TickerWS) parseBinaryResponse(resp []byte) ([]byte, error) {
...
func (w *TickerWS) findSymbol(msg []byte) string {
...
func NewTickerWS(
	...
	ws := TickerWS{
		...
		writeCh:     make(chan interface{}, len(channels)*100),
		symbolCh:    make(chan string, len(channels)*100),
