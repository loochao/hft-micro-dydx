...
type BncfBookTickerWS struct {
	done        chan interface{}
	reconnectCh chan interface{}
	stopped     int32
}

func (w *BncfBookTickerWS) readLoop(conn *websocket.Conn, channels map[string]chan *Message) {
	...
	for {
		...
		if msg[18] == '@' {
			...
			symbol = common.UnsafeBytesToString(msg[11:29])
		} else {
...
func (w *BncfBookTickerWS) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 1024)
	for {
		if len(b) == cap(b) {
...
func (w *BncfBookTickerWS) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *BncfBookTickerWS) mainLoop(ctx context.Context, proxy string, channels map[string]chan *Message) {
...
func (w *BncfBookTickerWS) heartbeatLoop(ctx context.Context, conn *websocket.Conn, symbols []string) {
...
func (w *BncfBookTickerWS) restart() {
	select {
	case <-w.done:
		return
	default:
...
func (w *BncfBookTickerWS) Done() chan interface{} {
	return w.done
}

func NewBncfBookTickerWS(
	ctx context.Context,
	proxy string,
