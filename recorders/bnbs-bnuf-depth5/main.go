...
func main() {
	...
	//savePath := flag.String("path", "/root/bnbs-bnuf-depth5", "data save folder")
	...
	for start := 0; start < len(symbols); start += *batchSize {
		...
		uMessageChMap := make(map[string]chan Message)
		bMessageChMap := make(map[string]chan Message)
		for _, bSymbol := range symbols[start:end] {
			uSymbol := strings.Replace(bSymbol, "BUSD", "USDT", -1)
			uMessageChMap[strings.ToLower(uSymbol)] = make(chan Message, 10000)
			bMessageChMap[strings.ToLower(bSymbol)] = uMessageChMap[strings.ToLower(uSymbol)]
			go saveLoop(ctx, cancel, *savePath, bSymbol, uSymbol, uMessageChMap[strings.ToLower(bSymbol)], fileSavedCh)
		}
		...
		}(ctx, cancel, *proxyAddress, uMessageChMap)
		go func(ctx context.Context, cancel context.CancelFunc, proxy string, outputChMap map[string]chan Message) {
