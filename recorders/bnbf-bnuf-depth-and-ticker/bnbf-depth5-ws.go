...
func (w *BnbfDepth5WS) readLoop(conn *websocket.Conn, channels map[string]chan *Message) {
...
func (w *BnbfDepth5WS) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 1024)
	for {
		if len(b) == cap(b) {
...
func (w *BnbfDepth5WS) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *BnbfDepth5WS) mainLoop(ctx context.Context, proxy string, channels map[string]chan *Message) {
...
func (w *BnbfDepth5WS) heartbeatLoop(ctx context.Context, conn *websocket.Conn) {
...
func (w *BnbfDepth5WS) Done() chan interface{} {
	return w.done
}

func NewBnbfDepth5WS(
	ctx context.Context,
	proxy string,
