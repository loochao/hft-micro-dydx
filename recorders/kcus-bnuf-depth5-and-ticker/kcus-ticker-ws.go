...
func (w *KcusTickerWS) writeLoop(ctx context.Context, conn *websocket.Conn) {
...
func (w *KcusTickerWS) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 512)
	for {
		if len(b) == cap(b) {
...
func (w *KcusTickerWS) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *KcusTickerWS) heartbeatLoop(ctx context.Context, conn *websocket.Conn, symbols []string, pingInterval time.Duration) {
...
func (w *KcusTickerWS) Done() chan interface{} {
	return w.done
}

func NewKcusTickerWS(
	ctx context.Context,
	proxy string,
	channels map[string]chan *Message,
) *KcusTickerWS {
