...
func main() {
	...
	savePath := flag.String("path", "/root/okus", "data save folder")
	...
	api, err := okexv5_usdtspot.NewAPI(&okexv5_usdtspot.Credentials{}, *proxyAddress)
	if err != nil {
		logger.Fatal(err)
	}
	var instruments []okexv5_usdtspot.Instrument
	...
	for _, instrument := range instruments {
		...
		_, ok1 := filterMap[strings.Replace(instrument.InstId, "-USDT", "USDT", -1)]
		if ok1  {
	...
	for start := 0; start < len(symbols); start += *batchSize {
		...
		okusChMap := make(map[string]chan *common.RawMessage)
		for _, xSymbol := range symbols[start:end] {
			okusChMap[xSymbol] = make(chan *common.RawMessage, 1024)
			go common.RawWSMessageSaveLoopForSingleSymbol(ctx, cancel, *savePath, xSymbol, okusChMap[xSymbol], fileSavedCh)
		}
		go func(ctx context.Context, cancel context.CancelFunc, proxy string, outputChMap map[string]chan *common.RawMessage) {
			ws1 := okexv5_usdtspot.NewRawDepth5WS(ctx, proxy, []byte{'D'}, outputChMap)
			ws2 := okexv5_usdtspot.NewRawTickerWS(ctx, proxy, []byte{'B'}, outputChMap)
			ws3 := okexv5_usdtspot.NewRawTradeWS(ctx, proxy, []byte{'T'}, outputChMap)
			select {
			case <-ctx.Done():
			case <-ws1.Done():
				cancel()
		...
		}(ctx, cancel, *proxyAddress, okusChMap)
	}
