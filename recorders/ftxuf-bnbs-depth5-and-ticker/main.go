...
func main() {
	...
	savePath := flag.String("path", "/root/ftxuf-bnbs-depth5-and-ticker", "data save folder")
	...
	for key := range ftx_usdfuture.PriceIncrements {
		if _, ok := binance_busdspot.TickSizes[strings.Replace(key, "-PERP", "BUSD", -1)]; ok {
	...
	for start := 0; start < len(symbols); start += *batchSize {
		...
		for _, xSymbol := range symbols[start:end] {
			ySymbol := strings.Replace(xSymbol, "-PERP", "BUSD", -1)
