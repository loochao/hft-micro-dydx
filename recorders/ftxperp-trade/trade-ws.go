			...
			for market, updateTime := range marketUpdatedTimes {
				if time.Now().Sub(updateTime) > symbolTimeout {
					...
					case w.writeCh <- ftx_usdfuture.SubscribeParam{
						...
						break loop
					default:
						logger.Debugf("w.writeCh <- ftx-usdfuture.SubscribeParam failed, ch len %d", len(w.writeCh))
					}
				}
			}
...
func (w *TradeWS) saveLoop(ctx context.Context, savePath, symbol string, inputCh chan []byte, fileSavedCh chan string) {
	...
	for {
		...
		case <-hourUpdateTimer.C:
			...
			outPath = fmt.Sprintf("%s/%s-%s.ftx-usdfuture.trade.jl.gz", savePath, dayTime.Format("20060102"), symbol)
