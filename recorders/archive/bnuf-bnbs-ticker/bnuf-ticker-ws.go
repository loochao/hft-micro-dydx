...
func (w *BnufTicker) readLoop(conn *websocket.Conn, channels map[string]chan Message) {
...
func (w *BnufTicker) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 1024)
	for {
		if len(b) == cap(b) {
...
func (w *BnufTicker) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *BnufTicker) mainLoop(ctx context.Context, proxy string, channels map[string]chan Message) {
...
func (w *BnufTicker) heartbeatLoop(ctx context.Context, conn *websocket.Conn) {
...
func (w *BnufTicker) Done() chan interface{} {
	return w.done
}
