...
func (w *BnbsTicker) readLoop(conn *websocket.Conn, channels map[string]chan Message) {
...
func (w *BnbsTicker) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 1024)
	for {
		if len(b) == cap(b) {
...
func (w *BnbsTicker) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *BnbsTicker) mainLoop(ctx context.Context, proxy string, channels map[string]chan Message) {
...
func (w *BnbsTicker) heartbeatLoop(ctx context.Context, conn *websocket.Conn) {
...
func (w *BnbsTicker) Done() chan interface{} {
	return w.done
}
