...
func saveLoop(ctx context.Context, cancel context.CancelFunc, savePath, symbol1, symbol2 string, messageInputCh chan Message, fileSavedCh chan string) {
	logger.Debugf("START saveLoop %s %s", symbol1, symbol2)
	...
	defer func() {
		if gw != nil {
			gw.Flush()
			gw.Close()
			logger.Debugf("close gzip writer for %s %s", symbol1, symbol2)
			gw = nil
		}
		if file != nil {
			logger.Debugf("close file writer for %s %s", symbol1, symbol2)
			file.Close()
			file = nil
		}
		fileSavedCh <- symbol1
		logger.Debugf("EXIT saveLoop %s %s", symbol1, symbol2)
		cancel()
	}()
	for {
		select {
		...
		case <-hourUpdateTimer.C:
			...
			outPath = fmt.Sprintf("%s/%s-%s,%s.ticker.jl.gz", savePath, dayTime.Format("20060102"), symbol1, symbol2)
		...
		case msg = <-messageInputCh:
			...
			if gw != nil {
				...
				_, err = gw.Write([]byte(fmt.Sprintf("%d", msg.Time)))
				if err != nil {
					cancel()
					logger.Debugf("gw.Write error %v, stop ws", err)
					return
