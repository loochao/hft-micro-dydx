...
func saveLoop(ctx context.Context, cancel context.CancelFunc, savePath, bSymbol, uSymbol string, messageInputCh chan Message, fileSavedCh chan string) {
	logger.Debugf("START saveLoop %s %s", bSymbol, uSymbol)
	...
	defer func() {
		if gw != nil {
			gw.Flush()
			gw.Close()
			logger.Debugf("close gzip writer for %s %s", bSymbol, uSymbol)
			gw = nil
		}
		if file != nil {
			logger.Debugf("close file writer for %s %s", bSymbol, uSymbol)
			file.Close()
			file = nil
		}
		fileSavedCh <- bSymbol
		logger.Debugf("EXIT saveLoop %s %s", bSymbol, uSymbol)
		cancel()
	}()
	for {
		select {
		...
		case <-hourUpdateTimer.C:
			...
			outPath = fmt.Sprintf("%s/%s-%s,%s.depth5.jl.gz", savePath, dayTime.Format("20060102"), bSymbol, uSymbol)
