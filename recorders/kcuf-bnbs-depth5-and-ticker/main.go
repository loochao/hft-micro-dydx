...
func main() {
	...
	savePath := flag.String("path", "/root/kcuf-bnbs-depth5-and-ticker", "data save folder")
	...
	symbolsMap["XBTUSDTM"] = "BTCBUSD"
	for key := range kucoin_usdtfuture.TickSizes{
		if _, ok := binance_busdspot.TickSizes[strings.Replace(key, "USDTM", "BUSD", -1)]; ok {
			symbols = append(symbols, key)
			symbolsMap[key] = strings.Replace(key, "USDTM", "BUSD", -1)
		}
	}
	//symbols = symbols[:1]
	...
	for start := 0; start < len(symbols); start += *batchSize {
		...
		for _, xSymbol := range symbols[start:end] {
			...
			bnbsChMap[strings.ToLower(ySymbol)] = kcufChMap[xSymbol]
