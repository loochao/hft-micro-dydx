...
type BybitUfOrderBookWS struct {
...
func (w *BybitUfOrderBookWS) writeLoop(ctx context.Context, conn *websocket.Conn) {
...
func (w *BybitUfOrderBookWS) readLoop(conn *websocket.Conn, channels map[string]chan []byte) {
...
func (w *BybitUfOrderBookWS) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 1024)
	for {
		if len(b) == cap(b) {
...
func (w *BybitUfOrderBookWS) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *BybitUfOrderBookWS) mainLoop(ctx context.Context, proxy string, channels map[string]chan []byte) {
...
func (w *BybitUfOrderBookWS) heartbeatLoop(ctx context.Context, conn *websocket.Conn, symbols []string) {
	...
	for {
		...
		case <-marketCheckTimer.C:
			...
			if len(args) > 0 {
				select {
				case w.writeCh <- bybit_usdtfuture.SubscribeParam{
					Op:   "unsubscribe",
					Args: args,
				}:
				default:
...
func (w *BybitUfOrderBookWS) Stop() {
...
func (w *BybitUfOrderBookWS) restart() {
	select {
...
func (w *BybitUfOrderBookWS) Done() chan interface{} {
	return w.done
}

func (w *BybitUfOrderBookWS) dataHandleLoop(ctx context.Context, symbol string, inputCh chan []byte, outputCh chan *Message) {
	...
	var resetTimer = time.NewTimer(time.Minute * 10)
	for {
		select {
		case <-ctx.Done():
			return
		...
		case <-resetTimer.C:
			...
			resetTimer.Reset(time.Minute * 10)
			break
		case msg = <-inputCh:
			...
			if hasPartial {
				...
				default:
					if time.Now().Sub(logSilentTime) > 0 {
						logger.Debugf(" ch <- msg %s ch len %d", symbol, len(outputCh))
						logSilentTime = time.Now().Add(time.Minute)
					}
				}
				select {
...
func NewBybitUfOrderBookWS(
	ctx context.Context,
	proxy string,
	channels map[string]chan *Message,
) *BybitUfOrderBookWS {
	ws := BybitUfOrderBookWS{
	...
	for market, ch := range channels {
		messagesCh[market] = make(chan []byte, 100)
