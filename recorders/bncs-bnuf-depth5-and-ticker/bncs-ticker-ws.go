...
func (w *BncsBookTickerWS) readLoop(conn *websocket.Conn, channels map[string]chan *Message) {
...
func (w *BncsBookTickerWS) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 1024)
	for {
		if len(b) == cap(b) {
...
func (w *BncsBookTickerWS) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *BncsBookTickerWS) mainLoop(ctx context.Context, proxy string, channels map[string]chan *Message) {
...
func (w *BncsBookTickerWS) heartbeatLoop(ctx context.Context, conn *websocket.Conn) {
...
func (w *BncsBookTickerWS) restart() {
	select {
	case <-w.done:
		return
	default:
...
func (w *BncsBookTickerWS) Done() chan interface{} {
	return w.done
}

func NewBncsBookTickerWS(
	ctx context.Context,
	proxy string,
