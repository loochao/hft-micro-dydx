...
func (w *BnusBookTickerWS) readLoop(conn *websocket.Conn, channels map[string]chan *Message) {
...
func (w *BnusBookTickerWS) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 1024)
	for {
		if len(b) == cap(b) {
...
func (w *BnusBookTickerWS) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *BnusBookTickerWS) mainLoop(ctx context.Context, proxy string, channels map[string]chan *Message) {
...
func (w *BnusBookTickerWS) heartbeatLoop(ctx context.Context, conn *websocket.Conn) {
...
func (w *BnusBookTickerWS) restart() {
	select {
	case <-w.done:
		return
	default:
...
func (w *BnusBookTickerWS) Done() chan interface{} {
	return w.done
}

func NewBnusBookTickerWS(
	ctx context.Context,
	proxy string,
