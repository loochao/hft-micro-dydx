...
func main() {
	...
	for _, symbol := range strings.Split(symbols, ",") {
		...
		loseCount := 0.0
		evenCount := 0.0
		var mircoPrice, midPrice, lastMidPrice float64
		for scanner.Scan() {
			...
			for i := 0; i < 20; i++ {
				...
				bidImpact += d.Bids[i][0] * d.Asks[i][1] * decay
				askImpact += d.Asks[i][0] * d.Bids[i][1] * decay
				decay *= 0.995
			}
			mircoPrice = (bidImpact + askImpact) / (bidSize + askSize)
			midPrice = (d.Bids[0][0] + d.Asks[0][0]) / 2
			fields := make(map[string]interface{})
			fields["bidSize"] = bidSize
			fields["askSize"] = askSize
			fields["spread"] = (d.Asks[0][0] - d.Bids[0][0]) / d.Asks[0][0]
			fields["mircoPrice"] = mircoPrice
			fields["midPrice"] = midPrice
			if predictDir == nil {
				dir := 0
				if mircoPrice > midPrice {
					dir = 1
				} else if mircoPrice < midPrice {
					dir = -1
				}
				predictDir = &dir
			} else {
				if bidSize > askSize*4 || askSize > bidSize*4 {
					if *predictDir > 0 && midPrice > lastMidPrice {
						winCount++
					} else if *predictDir >= 0 && midPrice < lastMidPrice {
						loseCount++
					} else if *predictDir < 0 && midPrice < lastMidPrice {
						winCount++
					} else if *predictDir <= 0 && midPrice > lastMidPrice {
				...
				fields["winRatio"] = winCount / (winCount + loseCount + evenCount)
				fields["loseRatio"] = loseCount / (winCount + loseCount + evenCount)
