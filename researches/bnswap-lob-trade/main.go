...
func main() {
	...
	for _, symbol := range strings.Split(symbols, ",") {
		...
		buyTimedVolume := NewTimedVolume(time.Second * 300)
		sellTimedVolume := NewTimedVolume(time.Second * 300)
		...
		for depthScanner.Scan() {
			...
			if buyTimedVolume.Len() < 100 || sellTimedVolume.Len() < 100 {
				continue
			}
			...
			if bidAskRatio > 1 && buyTimedVolume.Sum() > 1.25*sellTimedVolume.Sum() {
				if entrySize == -1 {
			...
			} else if askBidRatio > 1 && 1.25*buyTimedVolume.Sum() < sellTimedVolume.Sum() {
				if entrySize == 1 {
			...
			//} else if entrySize > 0 && buyTimedVolume.Sum() < sellTimedVolume.Sum() {
			...
			fields["buyTimedVolume"] = buyTimedVolume.Sum()
			fields["sellTimedVolume"] = sellTimedVolume.Sum()
			if entrySize != 0 {
