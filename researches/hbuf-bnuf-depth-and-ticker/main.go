...
func main() {
	...
	for _, xSymbol := range symbols {
		...
		for _, dateStr := range strings.Split(dateStrs, ",") {
			...
			file, err := os.Open(
				fmt.Sprintf("/home/clu/MarketData/bnbf-bnuf-depth-and-ticker/%s/%s-%s,%s.jl.gz", dateStr, dateStr, xSymbol, ySymbol),
			)
			if err != nil {
	...
	if err == nil {
		err = ioutil.WriteFile("/home/clu/Projects/hft-micro/researches/bnbf-bnuf-depth-and-ticker/configs/longTD", longBytes, 0755)
		if err != nil {
			logger.Debugf("%v", err)
		}
	}
	...
	if err == nil {
		err = ioutil.WriteFile("/home/clu/Projects/hft-micro/researches/bnbf-bnuf-depth-and-ticker/configs/shortTD", shortBytes, 0755)
		if err != nil {
			logger.Debugf("%v", err)
		}
	}
