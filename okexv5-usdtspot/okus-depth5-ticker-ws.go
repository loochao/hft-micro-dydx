...
func (w *DepthTicker5WS) writeLoop(ctx context.Context, conn *websocket.Conn) {
...
func (w *DepthTicker5WS) readLoop(conn *websocket.Conn, channels map[string]chan []byte) {
...
func (w *DepthTicker5WS) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 1024)
	for {
		if len(b) == cap(b) {
...
func (w *DepthTicker5WS) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *DepthTicker5WS) mainLoop(ctx context.Context, proxy string, channels map[string]chan []byte) {
...
func (w *DepthTicker5WS) heartbeatLoop(ctx context.Context, conn *websocket.Conn, symbols []string) {
...
func (w *DepthTicker5WS) Done() chan interface{} {
	return w.done
}

func (w *DepthTicker5WS) dataHandleLoop(ctx context.Context, symbol string, inputCh chan []byte, outputCh chan common.Ticker) {
	logSilentTime := time.Now()
	var err error
	var depth5 *Depth5
	index := -1
