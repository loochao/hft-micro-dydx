...
type UserWS struct {
	...
	CashBalancesCh chan []CashBalance
		...
		case msg := <-w.messageCh:
			...
			if commonCap.Data != nil {
				...
				case "balance_and_position":
					...
					for _, bd := range bps {
						select {
						case w.CashBalancesCh <- bd.BalData:
						default:
							if time.Now().Sub(logSilentTime) > 0 {
								logger.Debugf("w.BalancesCh <- balances failed ch len %d", len(w.BalancesCh))
								logSilentTime = time.Now().Add(time.Minute)
...
func (w *UserWS) mainLoop(ctx context.Context, proxy string) {
	...
	for {
		...
		case <-reconnectTimer.C:
			...
			conn, err := w.reconnect(internalCtx, "wss://ws.okx.com:8443/ws/v5/private", proxy, 0)
			if err != nil {
...
func (w *UserWS) heartbeatLoop(ctx context.Context, conn *websocket.Conn) {
	...
	for _, topic := range []string{"account", "balance_and_position", "orders"} {
...
func NewUserWebsocket(
	...
	ws := UserWS{
		...
		BalancesCh:     make(chan []Balance, 64),
		CashBalancesCh: make(chan []CashBalance, 64),
