...
func (okut *OkexV5UsdtSpot) StreamBasic(ctx context.Context, statusCh chan common.SystemStatus, usdtAccountCh chan common.Balance, commissionAssetValueCh chan float64, positionChMap map[string]chan common.Position, orderChMap map[string]chan common.Order) {
	...
	for {
		...
		case balances := <-userWS.BalancesCh:
			for _, wsBalance := range balances {
				...
				if ch, ok := positionChMap[symbol]; ok {
					...
					if ok && wsBalance.UTime.Sub(lastBalance.UTime) < 0 {
						continue
					}
					if !ok {
						balance := wsBalance
						...
						balancesMap[symbol].UTime = wsBalance.UTime
		...
		case balance := <-internalAccountCh:
			...
			//	lastBalance, ok := balancesMap[symbol]
			//	if ok && balance.UTime.Sub(lastBalance.UTime) < 0 {
			//		continue
			//	}
			//	balancesMap[symbol] = &balance
			//	if ch, ok := positionChMap[symbol]; ok {
...
func (okut *OkexV5UsdtSpot) watchAccount(
	...
	for {
		...
		case <-timer.C:
			subCtx, _ := context.WithTimeout(ctx, time.Minute)
			balance, err := okut.api.GetAccount(subCtx)
			if err != nil {
