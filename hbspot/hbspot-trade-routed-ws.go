...
func (w *TradeRoutedWS) mainLoop(ctx context.Context, proxy string, channels map[string]chan TradeDetail) {
...
func (w *TradeRoutedWS) dataHandleLoop(ctx context.Context, id int, channels map[string]chan TradeDetail) {
	logger.Debugf("START dataHandleLoop %d", id)
	defer logger.Debugf("EXIT dataHandleLoop %d", id)
	logSilentTime := time.Now()
	var ch chan TradeDetail
	var err error
	var ok bool
	for {
		select {
...
func NewTradeRoutedWS(
	ctx context.Context,
	proxy string,
	channels map[string]chan TradeDetail,
) *TradeRoutedWS {
	ws := TradeRoutedWS{
	...
	for i := 0; i < 4; i++ {
		cs := make(map[string]chan TradeDetail)
