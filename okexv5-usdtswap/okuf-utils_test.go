...
func TestParseFundingRate(t *testing.T) {
	msg := []byte(`{"arg":{"channel":"funding-rate","instId":"BTC-USDT-SWAP"},"data":[{"fundingRate":"-0.00003062","fundingTime":"1636848000000","instId":"BTC-USDT-SWAP","instType":"SWAP","nextFundingRate":"-0.00013"}]}`)
	//msg := []byte(`{"arg":{"channel":"funding-rate","instId":"BTC-USDT-SWAP"},"data":[{"fundingRate":"-0.00003062","fundingTime":"1636848000000","instId":"BTC-USDT-SWAP","instType":"SWAP","nextFundingRate":"-0.00013"}]}`)
	fr := FundingRate{}
	comCap := CommonCapture{}
	err := json.Unmarshal(msg, &comCap)
	...
	frs := make([]FundingRate, 0)
	err = json.Unmarshal(comCap.Data, &frs)
	if err != nil {
		t.Fatal(err)
	}
	assert.Len(t, frs, 1)
	err = ParseFundingRate(msg[69:], &fr)
	if err != nil {
		t.Fatal(err)
	}
	assert.Equal(t, frs[0].InstId, fr.InstId)
	assert.Equal(t, frs[0].NextFundingRate, fr.NextFundingRate)
	assert.Equal(t, frs[0].FundingRate, fr.FundingRate)
	assert.Equal(t, frs[0].FundingTime.Sub(fr.FundingTime), time.Duration(0))
	logger.Debugf("%v", fr)
}
