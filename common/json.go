...
func (walker *JsonWalker) Collect() []byte {
	return walker.data[walker.collectStart:walker.collectEnd]
}

func (walker *JsonWalker) CollectString() (s string) {
	return UnsafeBytesToString(walker.Collect())
}

func (walker *JsonWalker) CollectInt() (int64, error) {
	return ParseInt(walker.Collect())
}

func (walker *JsonWalker) CollectFloat() (float64, error) {
	return ParseFloat(walker.Collect())
}

func (walker *JsonWalker) Get(offset int) byte {
	return walker.data[walker.collectEnd+offset]
}

func (walker *JsonWalker) GetString(offset int) string {
	return UnsafeBytesToString(walker.data[walker.collectEnd : walker.collectEnd+offset])
}

func (walker *JsonWalker) Start() int {
	return walker.collectStart
}

func (walker *JsonWalker) End() int {
	return walker.collectEnd
}

func (walker *JsonWalker) Len() int {
	return walker.len
}

func (walker *JsonWalker) Advance(offset int) bool {
	walker.collectEnd += offset
	if walker.collectEnd > walker.len {
		walker.collectEnd = walker.len
		return false
	}
	return true
}

func (walker *JsonWalker) ResetStart() {
	walker.collectStart = walker.collectEnd
}

func (walker *JsonWalker) Find(b byte) bool {
	for walker.Advance(1) {
		if walker.Get(0) == b {
			return true
		}
	}
...
func NewJsonWalker(data []byte) JsonWalker {
	return JsonWalker{
		data:         data,
		collectStart: 0,
		collectEnd:   0,
...
func ParseDecimal(s []byte) (float64, error) {
	if len(s) > 18 {
		//如果数据过长，越界unit64, 用strconv解析
		return strconv.ParseFloat(UnsafeBytesToString(s), 64)
	}
	mantissa := uint64(0)
	exp := 0
	base := uint64(10)
	...
	for i := 0; i < len(s); i++ {
		...
		case c == '.':
			if sawDot {
				//如果有两个dot, 直接break switch, 用strconv解析
				break
			}
			sawDot = true
			dp = nd
