...
func (r *IntRing) MarshalJSON() ([]byte, error) {
...
func (r *IntRing) UnmarshalJSON(data []byte) error {
	aux := &struct {
		Head int   `json:"head"`
		Tail int   `json:"tail"`
		Buff []int `json:"buff"`
...
*/
func (r *IntRing) ContentSize() int {
	if r.head == -1 {
		return 0
	} else {
...
func (r *IntRing) setCapacity(size int) {
	if size == len(r.buff) {
		return
	}
	...
	if r.head == -1 {
		...
		part1 := make([]int, len(r.buff[:r.head+1]))
		copy(part1, r.buff[:r.head+1])
		part2 := make([]int, len(r.buff[r.tail:]))
