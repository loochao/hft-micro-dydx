...
func GetEaseFnByName(name string) EaseFunction {
	switch name {
	case "linear":
		return Linear
	case "inQuad":
...
func InElasticFunction(period float64) EaseFunction {
	p := period
...
func OutElasticFunction(period float64) EaseFunction {
	p := period
	return func(t float64) float64 {
...
func InOutElasticFunction(period float64) EaseFunction {
	p := period
	return func(t float64) float64 {
		t *= 2
		if t < 1 {
