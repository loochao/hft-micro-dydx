...
func (r *Float64Ring) MarshalJSON() ([]byte, error) {
...
func (r *Float64Ring) UnmarshalJSON(data []byte) error {
	aux := &struct {
		Head int       `json:"head"`
		Tail int       `json:"tail"`
		Buff []float64 `json:"buff"`
...
func (r *Float64Ring) mod(p int) int {
	v := p % len(r.buff)
...
*/
func (r *Float64Ring) ContentSize() int {
	if r.head == -1 {
		return 0
	} else {
...
*/
func (r *Float64Ring) Enqueue(i float64) {
...
*/
func (r *Float64Ring) Dequeue() *float64 {
	if r.head == -1 {
		return nil
	}
	v := r.get(r.tail)
...
Returns nil if the ring buffer is empty.
*/
func (r *Float64Ring) Peek() *float64 {
	if r.head == -1 {
		return nil
	}
	v := r.get(r.tail)
...
func (r *Float64Ring) setCapacity(size int) {
	if size == len(r.buff) {
		return
	}
	...
	if r.head == -1 {
		...
		part1 := make([]float64, len(r.buff[:r.head+1]))
		copy(part1, r.buff[:r.head+1])
		part2 := make([]float64, len(r.buff[r.tail:]))
...
//*/
//func (r *Float64Ring) Values() []float64 {
//	if r.head == -1 {
//		return []float64{}
//	}
