...
func (w *DepthWS) writeLoop(ctx context.Context, conn *websocket.Conn) {
...
func (w *DepthWS) findMarket(msg []byte) (string, error) {
	collectStart := 0
	collectEnd := 94
	msgLen := len(msg)
...
func (w *DepthWS) readLoop(conn *websocket.Conn, channels map[string]chan []byte) {
...
func (w *DepthWS) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 1024)
	for {
		if len(b) == cap(b) {
			// Add more capacity (let append pick how much).
...
func (w *DepthWS) heartbeatLoop(ctx context.Context, conn *websocket.Conn, markets []string) {
...
func (w *DepthWS) Done() chan interface{} {
	return w.done
}

func (w *DepthWS) dataHandleLoop(ctx context.Context, market string, inputCh chan []byte, outputCh chan common.Depth) {
