...
func (bn *BinanceUsdtFuture) StreamSymbolStatus(ctx context.Context, channels map[string]chan common.SymbolStatusMsg, batchSize int) {
	panic("implement me")
}

func (bn *BinanceUsdtFuture) GenerateClientID() string {
	return fmt.Sprintf("%d%04d", time.Now().Unix(), rand.Intn(10000))
}

func (bn *BinanceUsdtFuture) GetMinNotional(symbol string) (float64, error) {
	return 0.0, nil

}

func (bn *BinanceUsdtFuture) GetMinSize(symbol string) (float64, error) {
...
func (bn *BinanceUsdtFuture) GetStepSize(symbol string) (float64, error) {
...
func (bn *BinanceUsdtFuture) GetTickSize(symbol string) (float64, error) {
...
func (bn *BinanceUsdtFuture) StreamDepth(ctx context.Context, channels map[string]chan common.Depth, batchSize int) {
...
func (bn *BinanceUsdtFuture) StreamTrade(ctx context.Context, channels map[string]chan common.Trade, batchSize int) {
...
func (bn *BinanceUsdtFuture) watchSystemStatus(
	ctx context.Context,
...
func (bn *BinanceUsdtFuture) watchAccount(
	ctx context.Context,
...
func (bn *BinanceUsdtFuture) tryReBalanceBnb(
	...
	if err != nil {
		logger.Debugf("BNB bn.usApi.GetAccount(ctx) err %v", err)
		return
	}
	...
	logger.Debugf("BNB 1 spot balance usdt %f bnb %f", spotUSDTBalance, spotBnbBalance)
	...
	if spotUSDTBalance < 0.8*deltaValue &&
		...
		logger.Debugf("BNB transfer %f usdt to usdt spot", transferSize)
		...
		if err != nil {
			logger.Debugf("BNB NewFutureAccountTransfer usdt to spot error %v", err)
			return
		} else {
			logger.Debugf("BNB NewFutureAccountTransfer usdt to spot %f %v, wait 3 minutes", transferSize, resp.TranId)
			select {
			case <-ctx.Done():
				return
			case <-bn.done:
	...
	if deltaSize*0.8 > spotBnbBalance {
		...
		if size*price > binance_usdtspot.MinNotionals["BNBUSDT"]*1.2 {
			...
			if err != nil {
				logger.Debugf("BNB bn.usApi.SubmitOrder buy %f bnb error %v", size, err)
				return
			} else {
	...
	logger.Debugf("BNB transfer %f bnb to usdt future", transferSize)
	childCtx, _ := context.WithTimeout(ctx, time.Minute)
	resp, _, err := bn.usApi.NewFutureAccountTransfer(childCtx, binance_usdtspot.FutureAccountTransferParams{
		Asset:  "BNB",
	...
	if err != nil {
		logger.Debugf("BNB NewFutureAccountTransfer error %v", err)
	} else {
		logger.Debugf("BNB NewFutureAccountTransfer spot to usdt future %f %v", transferSize, resp.TranId)
	}
}
...
func (bn *BinanceUsdtFuture) cancelOrder(ctx context.Context, param common.CancelOrderParam, errCh chan common.OrderError) {
