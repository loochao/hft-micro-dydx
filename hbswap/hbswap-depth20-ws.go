...
func (w *Depth20Websocket) start(ctx context.Context, api *API, symbols []string, proxy string) {
	...
	for {
		...
		case <-reconnectTimer.C:
			...
			conn, err := w.reconnect(internalCtx, "wss://api.hbdm.com/linear-swap-ws", proxy, 0)
			if err != nil {
...
func (w *Depth20Websocket) maintainHeartbeat(ctx context.Context, conn *websocket.Conn, symbols []string) {
	...
	conn.SetPingHandler(func(msg string) error {
		logger.Debugf("HBSWAP DEPTH20 WS PingHandler %s", msg)
