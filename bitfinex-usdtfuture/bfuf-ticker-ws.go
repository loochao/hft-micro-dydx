...
func (w *TickerWS) readLoop(
	...
	channelId2Symbol := make(map[string]string)
	var index = 0
	var chanId string
outerLoop:
	for {
		...
		if msg[0] == '{' && msg[2] == 'e' {
			if msg[10] == 's' {
				...
				} else if subscribeEvent.ChanId != 0 && subscribeEvent.Pair != "" {
					channelId2Symbol[strconv.FormatInt(subscribeEvent.ChanId, 10)] = subscribeEvent.Pair
				}
			} else if msg[10] == 'i' {
			...
			for index+2 < msgLen {
				if msg[index] == ',' {
					chanId = common.UnsafeBytesToString(msg[1:index])
					if symbol, ok = channelId2Symbol[chanId]; ok {
...
func (w *TickerWS) parseTicker(msg []byte, ticker *Ticker) (err error) {
	...
	for end < msgLen {
		if msg[end] == ',' {
			value, err = common.ParseDecimal(msg[start:end])
			end++
			start = end
			switch counter {
			case 0:
				...
				break
			case 1:
				ticker.BidSize = value
				break
			case 2:
				ticker.Ask = value
				break
			case 3:
				ticker.AskSize = value
				return nil
			default:
				return fmt.Errorf("bad counter %d", counter)
			}
			counter++
		}
		end++
