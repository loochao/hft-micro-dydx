...
type XYSimplifiedTickerStats2 struct {
...
func (sl *XYSimplifiedTickerStats2) Stop() {
	if atomic.CompareAndSwapInt32(&sl.stopped, 0, 1) {
		close(sl.done)
	}
}

func (sl *XYSimplifiedTickerStats2) Start(ctx context.Context) {
...
func (sl *XYSimplifiedTickerStats2) UnmarshalJSON(data []byte) error {
	type Alias XYSimplifiedTickerStats2
	aux := &struct {
		*Alias
	}{
		Alias: (*Alias)(sl),
	...
	if err := json.Unmarshal(data, &aux); err != nil {
		...
		if sl.SpreadTD == nil {
			sl.SpreadTD = NewTimedTDigestWithCompression(sl.spreadTDLookback, sl.spreadTDSubInterval, sl.spreadTDCompression)
		} else {
			sl.SpreadTD.Lookback = sl.spreadTDLookback
...
type NewXYSimplifiedTickerStats2Params struct {
	XSymbol                     string
	YSymbol                     string
	SampleInterval              time.Duration
...
func NewXYSimplifiedTickerStats2(params NewXYSimplifiedTickerStats2Params) (*XYSimplifiedTickerStats2, error) {
	...
	sl := &XYSimplifiedTickerStats2{

		SpreadTD:            NewTimedTDigestWithCompression(params.SpreadTDLookback, params.SpreadTDSubInterval, params.SpreadTDCompression),
		spreadTDLookback:    params.SpreadTDLookback,
