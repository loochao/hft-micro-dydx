...
func (tm *TimedCovariance) Insert(timestamp time.Time, x, y float64) float64 {
...
func (tm *TimedCovariance) Values() ([]float64, []float64) {
	return tm.xs, tm.ys
}

func (tm *TimedCovariance) Times() []time.Time {
	return tm.times
}

func (tm *TimedCovariance) Len() int {
	return len(tm.times)
}

func (tm *TimedCovariance) Range() time.Duration {
	if len(tm.times) > 2 {
...
func (tm *TimedCovariance) Covariance() float64 {
	return tm.covariance
}

func NewTimedCovariance(lookback time.Duration) *TimedCovariance {
	return &TimedCovariance{
		lookback: lookback,
