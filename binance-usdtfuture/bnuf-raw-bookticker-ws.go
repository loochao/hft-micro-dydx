...
func (w *RawBookTickerWS) readLoop(conn *websocket.Conn, channels map[string]chan *common.RawMessage) {
...
func (w *RawBookTickerWS) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 1024)
	for {
		if len(b) == cap(b) {
...
func (w *RawBookTickerWS) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *RawBookTickerWS) mainLoop(ctx context.Context, proxy string, channels map[string]chan *common.RawMessage) {
...
func (w *RawBookTickerWS) heartbeatLoop(ctx context.Context, conn *websocket.Conn, symbols []string) {
...
func (w *RawBookTickerWS) restart() {
	select {
	case <-w.done:
		return
	default:
...
func (w *RawBookTickerWS) Done() chan interface{} {
	return w.done
}

func (w *RawBookTickerWS) dataHandleLoop(ctx context.Context, symbol string, inputCh chan []byte, outputCh chan common.Ticker) {
	logSilentTime := time.Now()
	var err error
	var msg []byte
