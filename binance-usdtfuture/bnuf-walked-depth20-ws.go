...
func (w *WalkedDepth20WS) readLoop(conn *websocket.Conn, channels map[string]chan []byte) {
...
func (w *WalkedDepth20WS) readAll(r io.Reader) ([]byte, error) {
	b := make([]byte, 0, 1024)
	for {
		if len(b) == cap(b) {
			// Add more capacity (let append pick how much).
...
func (w *WalkedDepth20WS) reconnect(ctx context.Context, wsUrl string, proxy string, counter int64) (*websocket.Conn, error) {
...
func (w *WalkedDepth20WS) mainLoop(ctx context.Context, proxy string, channels map[string]chan []byte) {
...
func (w *WalkedDepth20WS) heartbeatLoop(ctx context.Context, conn *websocket.Conn, symbols []string) {
...
func (w *WalkedDepth20WS) restart() {
	select {
	case <-w.done:
		return
	default:
...
func (w *WalkedDepth20WS) Done() chan interface{} {
	return w.done
}

func (w *WalkedDepth20WS) dataHandleLoop(ctx context.Context, symbol string, inputCh chan []byte, impact float64, outputCh chan common.Ticker) {
	logSilentTime := time.Now()
	var err error
	var msg []byte
	var walkedDepth20 *common.WalkedDepth
	index := -1
	...
	for {
		...
		case msg = <-inputCh:
			...
			walkedDepth20 = pool[index]
			err = common.WalkDepth(depth20, 1.0, impact, walkedDepth20)
			select {
			case outputCh <- walkedDepth20:
			default:
				if time.Now().Sub(logSilentTime) > 0 {
...
func NewWalkedDepth20WS(
	...
	for symbol, ch := range channels {
		...
		go ws.dataHandleLoop(ctx, symbol, messageChs[strings.ToLower(symbol)], impact, ch)
	}
