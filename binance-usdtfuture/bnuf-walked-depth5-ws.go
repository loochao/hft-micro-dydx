...
mainLoop:
	for {
		...
		if readCounter%1000000 == 0 {
			logger.Debugf("BNUF WALKED DEPTH5 READ SIZE %d TOTAL %d PARTIAL %d ALLOCATE %d", depth5ReadMsgSize, readCounter, partialReadCounter, allocateCounter)
		}
		msgLen = len(msg)
		if msgLen < 128 {
			continue mainLoop
...
func (w *WalkedDepth5WS) mainLoop(ctx context.Context, proxy string, channels map[string]chan []byte) {
...
func (w *WalkedDepth5WS) heartbeatLoop(ctx context.Context, conn *websocket.Conn, symbols []string) {
