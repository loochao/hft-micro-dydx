...
func (w *RawTradeWS) readLoop(
	...
	for {
		...
		if msgLen > 128 {
			if msg[2] == 't' && msg[9] == 'm' {
				if msg[49] == '"' {
					symbol = common.UnsafeBytesToString(msg[41:49])
				} else if msg[50] == '"' {
					symbol = common.UnsafeBytesToString(msg[41:50])
				} else if msg[51] == '"' {
					symbol = common.UnsafeBytesToString(msg[41:51])
				} else if msg[52] == '"' {
					symbol = common.UnsafeBytesToString(msg[41:52])
				} else if msg[48] == '"' {
					symbol = common.UnsafeBytesToString(msg[41:48])
				} else {
			...
			for symbol, updateTime := range symbolUpdatedTimes {
				if time.Now().Sub(updateTime) > symbolTimeout {
					...
					case <-ctx.Done():
						return
					case <-time.After(time.Millisecond):
						logger.Debugf("send msg to writeCh timeout in 1m, %s", fmt.Sprintf("/market/match:%s", symbol))
