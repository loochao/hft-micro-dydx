			...
			if ch, ok = channels[symbol]; ok {
				...
				default:
					if time.Now().Sub(logSilentTime) > 0 {
						logger.Debugf("ch <- &common.TickerRawMessage failed %s len(ch) %d", symbol, len(ch))
...
func (w *Depth5TickerWS) mainLoop(ctx context.Context, api *API, proxy string, channels map[string]chan []byte) {
