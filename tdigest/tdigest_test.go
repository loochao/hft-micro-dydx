...
func TestTInternals(t *testing.T) {
	...
	if !math.IsNaN(tdigest.Quantile(0.1)) {
		t.Errorf("GridQuantile() on an empty digest should return NaN. Got: %.4f", tdigest.Quantile(0.1))
	}
	...
	if tdigest.Quantile(0.1) != 0.4 {
		t.Errorf("GridQuantile() on a single-sample digest should return the samples's mean. Got %.4f", tdigest.Quantile(0.1))
	}
...
func assertDifferenceSmallerThan(tdigest *TDigest, p float64, m float64, t *testing.T) {
	...
	if math.Abs(tp-p) >= m {
		t.Errorf("T-Digest.GridQuantile(%.4f) = %.4f. BaseDiff (%.4f) >= %.4f", p, tp, math.Abs(tp-p), m)
	}
}
...
func assertDifferenceFromQuantile(data []float64, tdigest *TDigest, p float64, m float64, t *testing.T) {
	...
	if math.Abs(tp-q) >= m {
		t.Fatalf("T-Digest.GridQuantile(%.4f) = %.4f vs actual %.4f. BaseDiff (%.4f) >= %.4f", p, tp, q, math.Abs(tp-q), m)
	}
}
...
func TestSingletonInACrowd(t *testing.T) {
	...
	for _, q := range []float64{0, 0.5, 0.8, 0.9, 0.99, 0.999} {
		...
		if !closeEnough(result, 10) {
			t.Errorf("Expected GridQuantile(%.3f) = 10, but got %.4f (size=%d)", q, result, tdigest.summary.Len())
		}
	}

	result := tdigest.Quantile(1)
	if result != 20 {
		t.Errorf("Expected GridQuantile(1) = 20, but got %.4f (size=%d)", result, tdigest.summary.Len())
	}
}
...
func TestGammaDistribution(t *testing.T) {
	...
	for _, q := range []float64{0.001, 0.01, 0.1, 0.5, 0.9, 0.99, 0.999} {
		...
		if error > 0.012 {
			t.Errorf("Error in estimated GridQuantile too high")
		}
	}
...
func TestPanic(t *testing.T) {
	...
	shouldPanic(func() {
		tdigest.Quantile(-42)
	}, t, "GridQuantile < 0 should panic!")

	shouldPanic(func() {
		tdigest.Quantile(42)
	}, t, "GridQuantile > 1 should panic!")
}
